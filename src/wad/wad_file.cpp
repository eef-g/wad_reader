...
namespace wad {
    ...
    WADFile::WADFile(std::string path, std::string map) {
        this->reader = NULL;
        this->ReadWad(path);
    }
    ...
    // Public Functions
    void WADFile::ReadWad(std::string path) {
        if(this->reader != NULL) {
            delete this->reader;
        }
    ...
    void WADFile::ReadMap(std::string map) {
        this->map_index = this->GetLumpIndex(map);
    ...
    int WADFile::GetLumpIndex(std::string lump_name) {
    ...
    std::vector<THING> WADFile::ReadThingLump() {
    ...
    std::vector<LINEDEF> WADFile::ReadLinedefLump() {
    ...
    std::vector<SIDEDEF> WADFile::ReadSidedefLump() {
    ...
    std::vector<VERTEX> WADFile::ReadVertexLump() {
        ...
        for(int i = 0; i < count; i++) {
            offset = lump_info.lump_pos + i * num_bytes + header_length;
            VERTEX curr_data = this->reader->ReadVertex(offset);
            data.push_back(curr_data);
        }
        return data;
    }

    std::vector<SEG> WADFile::ReadSegLump() {
    ...
    std::vector<SUBSECTOR> WADFile::ReadSubsectorLump() {
    ...
    std::vector<NODE> WADFile::ReadNodeLump() {
    ...
    std::vector<SECTOR> WADFile::ReadSectorLump() {
    ...
    std::vector<REJECT> WADFile::ReadRejectLump() {
    ...
    std::vector<BLOCKMAP> WADFile::ReadBlockmapLump() {
