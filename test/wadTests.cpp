// This test file is a collection of unit tests to ensure that the WADFile class is working as intended.
// To get the test values, the doom1.wad file is used. The map E1M1 is used for the tests.
// The values that are tested are the first values of each lump, and the size of each lump.
// The first values of each lump were cross-referenced with the values in the Doom Wiki & from the Doom level editor tool -- SLADE.
...
#pragma region wadFileTests
    ...
    // Define static member outside the class -- This is what we use in all of the tests when we refer to wad_file
